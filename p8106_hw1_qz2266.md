...
-   To fix the potential multicollinearity issue, regularization methods
    ...
    employed, other than linear model. Please see below for these
    models.
...
-   This model is quite straightforward and easy to fit.The estimates
    are BLUE.The mean-square test error (MSE) for this model is
    4.4728765^{8}.
...
![](p8106_hw1_qz2266_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->
...
-   The selected tuning parameters of elastic net model 1 is alpha =
    0.05 and lambda = 582.5 and test error is 4.3859117^{8}. If 1se rule
    is applied to elastic net model, the tuning parameters is alpha = 0
    and lambda is 2980.96, and test error is 4.2635771^{8} .

-   As we know, elastic net allows us to tune the alpha parameter where
    𝞪 = 0 corresponds to ridge and 𝞪 = 1 to lasso. That means we can
    choose an alpha value between 0 and 1 to optimize the elastic net.
    ...
    rule to select tuning parameters in this elastic net model. It
    doesn’t help to optimize this model.
...
![](p8106_hw1_qz2266_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
of components is 12. The test error is 4.4962272^{8}.
...
![](p8106_hw1_qz2266_files/figure-gfm/boxplot-1.png)<!-- -->
...
-   As for the rest 3 models, from the summary and boxplot we found
    elastic net model has the lowest RMSE, lowest MAE, as well as
    highest R_squred. In addition, it’s more difficult to interpret the
    results of partial least squares model.
