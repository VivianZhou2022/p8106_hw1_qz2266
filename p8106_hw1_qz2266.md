...
problem. - To fix the potential multicollinearity issue, regularization
...
BLUE.The mean-square test error (MSE) for this model is 4.4728765^{8}. -
However, this model is still quite complicated with too many predictors.
Moreover, there is multicollinearity issue and potential overfitting
problem.
...
From the summary of this partial least squares model we can find the
number of components is 12. The test error is 4.4962272^{8}.
